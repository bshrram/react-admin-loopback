...
var authProvider = function authProvider(loginApiUrl) {
  ...
  return function (type, params) {
    ...
    if (type === _reactAdmin.AUTH_GET_PERMISSIONS) {
      var role = _storage2.default.load('lbtoken');
      if (role && role.dashboardUser && role.dashboardUser.realm) {
        return Promise.resolve(role.dashboardUser.realm);
      } else {
        _storage2.default.remove('lbtoken');
        return Promise.reject({ redirecTo: noAccessPage });
      }

      //const role = JSON.parse(localStorage.getItem('lbtoken')).user.realm;
